...
const getSummonersData = async (name) => {
    try {
        name = name.replaceAll(' ', '%20');
        const summonerDataResponse = await axios.get(`https://euw1.api.riotgames.com/lol/summoner/v4/summoners/by-name/${name}?api_key=${process.env.API_KEY}`);
        const summonerData = summonerDataResponse.data;
        const puuid = summonerData['puuid'];
        const id = summonerData['id'];
        const summonerLeagueResponse = await axios.get(`https://euw1.api.riotgames.com/tft/league/v1/entries/by-summoner/${id}?api_key=${process.env.API_KEY}`);
        ...
        const last20MatchesData = await getPreviousMatchesData(puuid, 1, false);
...
const getPreviousMatchesData = async (puuid, count, generalData) => {
    ...
    for (const matchId of matchesId) {
        ...
        if (!generalData) {
            const otherCompositions = await Promise.all(participants.map(async (item) => {
                if (item['puuid'] != puuid) {
                    let eliminated;
                    const nameResponse = await axios.get(`https://euw1.api.riotgames.com/lol/summoner/v4/summoners/by-puuid/${item['puuid']}?api_key=${process.env.API_KEY}`);
                    const name = nameResponse.data['name'];
                    if (item['last_round'] <= 3) {
                        eliminated = `1-${item['last_round']}`;
                    }
                    else {
                        eliminated = `${1 + Math.ceil((item['last_round'] - 3) / 7)}-${(item['last_round'] - 3) % 7 == 0
                            ? 7
                            : (item['last_round'] - 3) % 7}`;
                    }
                    const result = {
            ...
            const comps = {
                ...
                playedOn: new Date(matchData['info']['game_datetime'] * 1000).toUTCString()
            };
            allComps.push(comps);
        }
...
const getDetailedLeagueInfoData = async (id, tier, division, lp) => {
    ...
    if (tier == 'CHALLENGER' || tier == 'MASTER' || tier == 'GRANDMASTER') {
        const leagueResponse = await axios.get(`https://euw1.api.riotgames.com/tft/league/v1/${tier.toLowerCase()}?api_key=${process.env.API_KEY}`);
        ...
        if (tier == 'MASTER') {
            const grandmasterResponse = await axios.get(`https://euw1.api.riotgames.com/tft/league/v1/grandmaster?api_key=${process.env.API_KEY}`);
            peopleWithHigherLp += grandmasterResponse.data['entries'].length;
            const challengerResponse = await axios.get(`https://euw1.api.riotgames.com/tft/league/v1/challenger?api_key=${process.env.API_KEY}`);
        ...
        const grandmasterResponse = await axios.get(`https://euw1.api.riotgames.com/tft/league/v1/grandmaster?api_key=${process.env.API_KEY}`);
        peopleWithHigherLp += grandmasterResponse.data['entries'].length;
        const masterResponse = await axios.get(`https://euw1.api.riotgames.com/tft/league/v1/master?api_key=${process.env.API_KEY}`);
        peopleWithHigherLp += masterResponse.data['entries'].length;
        let isFinished = false;
        let pageCount = 1;
        do {
            let currentLeagueResponse = await axios.get(`https://euw1.api.riotgames.com/tft/league/v1/entries/${tier}/${division}?page=${pageCount}&api_key=${process.env.API_KEY}`);
...
app.get('/summoner/:name', async (req, res) => {
    return getSummonersData(req.params.name);
});
