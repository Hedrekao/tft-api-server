...
const saveCompositionIntoDatabase = async (composition) => {
    try {
        const result = await getItemsAndAugmentsAndVariationsPerformance(composition);
        if (result.error) {
        ...
        if (result.augmentData && result.itemsData) {
            saveAugmentsAndItemsIntoDatabase(comp.id, result.augmentData, result.itemsData, composition.units, prisma);
        }
    }
    catch (error) {
