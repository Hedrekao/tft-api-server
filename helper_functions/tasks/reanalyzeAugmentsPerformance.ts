...
export async function reanalyzeAugmentsPerformance(
  ...
  for (const augmentData in augmentsData) {
    ...
    const currentAugment = currentCompAugments.find(
      (augment) => augment.augmentId == augmentData
    );

    if (currentAugment) {
      avgPlace = parseFloat(
        (
          (augmentSumOfPlacements + currentAugment.sumOfPlacements) /
          (augmentNumberOfComps + currentAugment.numOfAppear)
        ).toFixed(2)
      );
      winRate = parseFloat(
        (
          ((augmentNumberOfWins + currentAugment.numOfWins) /
            (augmentNumberOfComps + currentAugment.numOfAppear)) *
          100
        ).toFixed(2)
      );
      ...
      avgPlace = parseFloat(
        (augmentSumOfPlacements / augmentNumberOfComps).toFixed(2)
      );
      winRate = parseFloat(
        ((augmentNumberOfWins / augmentNumberOfComps) * 100).toFixed(2)
      );
      playRate = parseFloat(
        ((augmentNumberOfComps / numberOfMatchingComps) * 100).toFixed(2)
      );
